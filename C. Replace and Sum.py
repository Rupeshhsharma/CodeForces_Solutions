...
Today, KQ has an exam at the Grail Academy. A strict teacher gave a task that KQ could not solve. He was given two arrays a and b of length n
...
Now he has q queries. Each query is described by two numbers l and r. His task is to find the maximum value of the sum (al+al+1+al+2+⋯+ar)
...
Output
For each test case, output q numbers separated by spaces — the maximum values of the sums (al+al+1+al+2+⋯+ar).
...
################################################################Solution########################################################
add the maximum number in the array then according to l and r so the prefix 
----------------------------------------------------------------Code-----------------------------------------------------
t = int(input())
for _ in range(t):
