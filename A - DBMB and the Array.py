...
DBMB had a birthday yesterday. He was gifted an array a of n elements and a number x. But there is one problem: he only likes arrays where the sum of the 
elements equals s. To make the array appealing to him, you can perform the following operation any number of times:
...
################################################################Solution####################################################
sum(x)+kn=s
we need to find the k if k is in fraction then it is not possible to make s
k=s-x%n
-----------------------------------------------------Code-----------------------------------------------
t=int(input())
for i in range(t):
    a,s,x=map(int, input().split())
